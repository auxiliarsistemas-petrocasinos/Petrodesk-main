...
export default function Assets() {
  ...
  const [createForm, setCreateForm] = useState({ internalCode: '', serial: '', brand: '', model: '', status: 'AVAILABLE', fieldId: '' })
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      if (editMode && editingId) {
        await api.patch(`/assets/${editingId}`, createForm)
        showToast('Activo actualizado', 'success')
      } else {
        await api.post('/assets', createForm)
  ...
  const resetForm = () => {
    setCreateForm({ internalCode: '', serial: '', brand: '', model: '', status: 'AVAILABLE', fieldId: '' })
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400">Administración de equipos, herramientas y recursos tecnológicos</p>
        </div>
        <button
      ...
      <div className="flex flex-col md:flex-row gap-3 p-4 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/60 dark:border-slate-800 shadow-sm">
        ...
        {(statusFilter || searchQuery) && (
          <button onClick={() => { setStatusFilter(''); setSearchQuery(''); setPage(1) }}
      ...
      {loading && assets.length === 0 ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-slate-500 dark:text-slate-400 max-w-sm mb-6">
            {searchQuery || statusFilter ? 'No se encontraron activos con los filtros seleccionados.' : 'Registra el primer activo del inventario.'}
          </p>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 dark:border-slate-800">
                  <th className="text-left px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Código / Modelo</th>
                  <th className="text-left px-4 py-4 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Estado</th>
                  <th className="text-left px-4 py-4 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Marca</th>
              ...
              <tbody>
                {assets.map((asset: any) => (
                  <tr key={asset.id} onClick={() => openDetail(asset.id)}
                    ...
                    <td className="px-6 py-4">
                      <div className="font-bold text-slate-800 dark:text-white group-hover:text-[#FF6A23] transition-colors">{asset.internalCode}</div>
                      <div className="text-xs text-slate-400 dark:text-slate-500">{asset.brand} {asset.model}</div>
                    </td>
                    <td className="px-4 py-4"><StatusBadge status={asset.status} /></td>
                    <td className="px-4 py-4 text-slate-600 dark:text-slate-300">{asset.brand}</td>
                    <td className="px-4 py-4">
                      <span className="text-slate-600 dark:text-slate-300 text-sm">{getUserDisplayName(asset.assignedUser)}</span>
                    </td>
                    <td className="px-4 py-4 text-slate-500 dark:text-slate-400 text-sm">{asset.field?.name || '—'}</td>
      ...
      {/* Create / Edit Modal */}
      <Modal isOpen={isCreateOpen} onClose={resetForm} title={editMode ? 'Editar Activo' : 'Nuevo Activo'}>
        <form onSubmit={handleCreate} className="space-y-5">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1.5">Código Interno *</label>
              <input required value={createForm.internalCode}
                onChange={e => setCreateForm(f => ({ ...f, internalCode: e.target.value }))}
            ...
            <div>
              <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1.5">Serial *</label>
              <input required value={createForm.serial}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1.5">Marca *</label>
              <input required value={createForm.brand}
            ...
            <div>
              ...
              <input required value={createForm.model}
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select value={createForm.fieldId}
      ...
      {/* Detail Modal */}
      <Modal isOpen={!!selectedAsset} onClose={() => setSelectedAsset(null)} title={selectedAsset ? `${selectedAsset.brand} ${selectedAsset.model}` : 'Detalle'}>
        {detailLoading ? (
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div><span className="text-slate-400 dark:text-slate-500">Serial:</span> <b className="text-slate-700 dark:text-slate-200 ml-1">{selectedAsset.serial}</b></div>
              <div><span className="text-slate-400 dark:text-slate-500">Marca:</span> <b className="text-slate-700 dark:text-slate-200 ml-1">{selectedAsset.brand}</b></div>
              <div><span className="text-slate-400 dark:text-slate-500">Modelo:</span> <b className="text-slate-700 dark:text-slate-200 ml-1">{selectedAsset.model}</b></div>
              <div><span className="text-slate-400 dark:text-slate-500">Campo:</span> <b className="text-slate-700 dark:text-slate-200 ml-1">{selectedAsset.field?.name || '—'}</b></div>
            </div>
            ...
            <div className="flex flex-wrap gap-3 p-4 bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800">
              <div className="flex-1 min-w-[140px]">
                ...
                <select value={selectedAsset.status} onChange={e => handleStatusChange(selectedAsset.id, e.target.value)}
              ...
              <div className="flex-1 min-w-[140px]">
                ...
                <select value={selectedAsset.assignedUserId || ''} onChange={e => handleAssign(selectedAsset.id, e.target.value)}
                  className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-[#FF6A23]/30">
            ...
            <button onClick={() => openEditModal(selectedAsset)}
              className="flex items-center gap-2 px-4 py-2 text-sm font-bold text-[#FF6A23] hover:bg-orange-50 dark:hover:bg-orange-950/20 rounded-xl transition-colors">
